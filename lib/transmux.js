...
import { fragmentedToProgressive } from './mp4.js';
...
//
// This is mux.js's native output: fragmented MP4 built for Media Source
// Extensions. It is not suitable for saving as a file — see tsToMp4.
export function tsToFragmentedMp4(input) {
...
// Transmux to a progressive MP4 — the format to actually save. mux.js only
// speaks fragmented MP4, which has no real duration in its header and no sample
// table, so players show a nonsense length and can't seek. fragmentedToProgressive
// rebuilds the sample tables from the fragments without touching the media.
export function tsToMp4(input) {
  return fragmentedToProgressive(tsToFragmentedMp4(input));
}
